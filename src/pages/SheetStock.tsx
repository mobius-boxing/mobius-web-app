...
const SheetStockPage: React.FC = () => {
  ...
  const canEdit = has('sheet-stock.edit');
  ...
  const filterDefs = useMemo(
    () => [
      searchFilter(t('sheetStock.searchPlaceholder')),
      ...columnFilterDefs('sheet-stock', columns, t, { companyId: effectiveCompanyId }),
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="gd-page-title">{t('sheetStock.title')}</h1>
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-medium text-secondary-900">
                {t('sheetStock.allSheetStock')} ({sheetStock.length})
              </h2>
            </div>

            {!loading && sheetStock.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="gd-page-sub">
                  {search ? t('sheetStock.empty.description') : t('sheetStock.empty.noData')}
                </p>
                {!search && canEdit && (
                  <div className="mt-6">
